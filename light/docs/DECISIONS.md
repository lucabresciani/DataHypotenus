...
## D-10 · CSS scritto a mano invece di un framework

**Decisione.** L'interfaccia usa CSS con variabili (design token) e nessun font remoto.

**Motivazione.** Il requisito è che l'applicazione funzioni offline: un font di Google
bloccherebbe il rendering senza rete. Un framework CSS avrebbe aggiunto una dipendenza di
build e un vincolo di aggiornamento per un'applicazione con una ventina di componenti. Il
sistema di token (colori, spazi, tipografia, ombre, z-index) sta in un file solo ed è più
piccolo della configurazione che servirebbe per personalizzare un framework.

**Alternative.** Tailwind: ottimo su progetti grandi, qui avrebbe portato un passo di build in
più e classi al posto di regole. Libreria di componenti pronta: l'aspetto "pannello
amministrativo generico" era esattamente ciò che il requisito escludeva.
